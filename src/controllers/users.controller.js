...
const login =async (req,res)=>{    
    ...
    req.session.user = new TokenDTO({first_name:req.user.name,email:req.user.email,role:req.user.role,id:req.user.id,cart:req.user.cart})   
...
const restoreRequest = async (req,res) =>{
    ...
    if (!user) return res.send({status:"no success"}) // email no valido
